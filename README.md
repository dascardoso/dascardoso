## <>Gabriel Cardoso</>
👾 Fullstack developer and cybersecurity enthusiast, passionate about 'breaking' and rethinking.  

---

## <>About Me</>
- Aspiring computer scientist and cybersecurity researcher  
- Interests: Artificial Intelligence, Databases and IT for Forensics.
- Passionate about making tech helpful and accessible for people who need it.   

---

## <>Current Projects</>
- AI Assistant tool for brazilian lawyers who often find difficulties dealing with the government systems.
- Visual Algorythm Simulator.
- "Don't Scream" - a roblox horror game.
- "Octaned" - a live chat system with websockets.
- Working Spotify Clone.
- Volunteer Developer for ONG Gabriel, a brazilian non-profitable organization for suicide prevention.
- Developer and Security Analist at [hsyst.xyz](#https://hsyst.xyz/html/hub.html), a non-profitable organization between friends that distributes open-source projects for the community.

---

## <>Tech Stack</>
**Languages:** Python, C, JavaScript, Lua, Ruby, TypeScript
**Frameworks:** React, Node.js, Express 
**Tools:** Git, Docker, Prisma, PostgreSQL, MySQL, Kali(Linux), Flask
**Other:** Linux, Data Structures, Algorithms, APIs  
**Certificates:** 
-Alumni Alumni;
-Harvard's CS50 Introduction to Computer Science
-Harvard's CS50 Artifficial Intelligence With Python
-Advanced Python Programming at CursoEmVideo (120h)

---

## <>Featured Projects</>
- [**Bridge OS**](#) – Open-source Linux Distro at hsyst organization  
- [**Playstube**](#) – Open-source video hosting platform, organized in folders. Ideal for those looking for a self-hosted alternative to YouTube.
- [**LivePub**](#) – Self-hosted live streaming solution. Allows companies and creators to have their own streaming environment without relying on external platforms.  

---

## <>Connect</>
I'm always open to help on any project and to make new connections!
- 💼 [LinkedIn](#) Temporarily offline
- 🌐 [Personal Website](#) Work in progress...
- 📧 [Email](#mailto:gabriel.nepcardoso@gmail.com)
- 📞 [Whatsapp](#wa.me/5527997633482)

---

"Technology can - and should - be used to subvert any negative status quo on all possible fields, so it's our duty to find out how, and make it happen.*  
